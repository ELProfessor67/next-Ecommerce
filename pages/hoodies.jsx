				...
				<section className="text-gray-600 body-font min-h-full">
				  <div className="container px-5 py-24 mx-auto">
				    <div className="flex flex-wrap -m-4 justify-center">
					    ...
					    }) : <p>sorry no hoodies yet hoodies are out of stock</p>}
					</div>
				  </div>
				</section>
			</>
	...
	const hoods = {};
	  for(let item of product){
	    if(item.title in hoods){
	      if(!hoods[item.title].color.includes(item.color) && item.available > 0){
	        hoods[item.title].color.push(item.color);
	      }
	      if(!hoods[item.title].size.includes(item.size) && item.available > 0){
		...
		props : {product : JSON.parse(JSON.stringify(hoods))}
	});
}