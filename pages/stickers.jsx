...
export default function stickers({product}){
	return(
			<>
				<section className="text-gray-600 body-font min-h-full min-h-full">
				  <div className="container px-5 py-24 mx-auto">
				    <div className="flex flex-wrap -m-4 justify-center">
					    ...
					    }) : <p>sorry no stickers yet stickers are out of stock</p>}
					</div>
				  </div>
				</section>
			</>
...
export async function getServerSideProps(){
	...
	const product = await Product.find({category : 'stickers'});
	const sticker = {};
	  for(let item of product){
	    if(item.title in sticker){
	      if(!sticker[item.title].color.includes(item.color) && item.available > 0){
	        sticker[item.title].color.push(item.color);
	      }
	      if(!sticker[item.title].size.includes(item.size) && item.available > 0){
		...
		props : {product : JSON.parse(JSON.stringify(sticker))}
	});
}