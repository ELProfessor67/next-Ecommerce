...
export default function Tshirt({product}){
	return(
			<>
				<section className="text-gray-600 body-font min-h-full">
				  <div className="container px-5 py-24 mx-auto">
				    <div className="flex flex-wrap -m-4 justify-center">
					    ...
					    }) : <p>sorry no tshirt yet tshirt are out of stock</p>}
					</div>
				  </div>
				</section>
			</>
	  ...
	  for(let item of product){
	    if(item.title in shirt){
	      ...
	      if(item.available > 0){
			...
			shirt[item.title].color = [];
			shirt[item.title].size = [];
		  }
	    }
  }	return({
		props : {product : JSON.parse(JSON.stringify(shirt))}
	});
}